...
test('add · div · pct 를 계산한다 — % 는 분모에 붙는다', () => {
  ...
  assert.equal(execute(e, { db, grid, year: '2025' }), 48);
});

test('pct 단독은 ÷100 이다', () => {
  const db = fixture();
  assert.equal(execute({ op: 'pct', inner: { op: 'const', v: 50 } },
    { db, grid, year: '2025' }), 0.5);
});
...
- Modify: `src/verify/compare.ts` (`Verdict` 에 `'presentation'` 추가 — Step 9)
- Modify: `scripts/verify-all.ts` (`presentation` 집계. **`gatePassed` 와 일치율 분모에서 뺀다** — 대조한 것이 아니다)
- Create: `data/manual/convert.json` (수기 wide 시트 변환표)
- Modify: `test/parse.test.ts` · `test/execute.test.ts` · `test/compare.test.ts` · `test/verify-all.test.ts`
- Modify: `reports/*`
